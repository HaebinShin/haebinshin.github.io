...
각 노드 별로 feature가 m차원일 때, 입력 행렬 $X$는 $X \in \mathbb{R}^{n \times m}$를 따르고, 각 vector는 $x_v \in \mathbb{R}^m$와 같다.  

GCN에서는 $G$의 인접행렬 $A$에 대해 degree matrix $D$를 사용한다. ($D_{ii}=\sum_{j}A_{ij}$)  
...
PMI 값이 양수일 경우 단어 간 high semantic correlation을 의미하고, 음수일 경우 semantic correlation 낮음을 의미한다. 따라서 인접행렬 $A$에서는 양수 값만을 사용한다.
